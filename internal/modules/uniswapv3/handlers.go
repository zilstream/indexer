...
func (m *UniswapV3Module) registerEventHandlers() error {
	...
	if ev, ok := m.poolABI.Events["Initialize"]; ok {
		m.handlers[ev.ID] = handleInitialize
	}
	if ev, ok := m.poolABI.Events["Swap"]; ok {
		m.handlers[ev.ID] = handleSwap
	}
	...
	if ev, ok := m.poolABI.Events["Mint"]; ok {
		m.handlers[ev.ID] = handleMint
	}
	if ev, ok := m.poolABI.Events["Burn"]; ok {
		m.handlers[ev.ID] = handleBurn
	}
	if ev, ok := m.poolABI.Events["Collect"]; ok {
		m.handlers[ev.ID] = handleCollect
	}
	return nil
}
...
func handlePoolCreated(ctx context.Context, module *UniswapV3Module, event *core.ParsedEvent) error {
	...
	if !ok {
		return fmt.Errorf("invalid token0 address in PoolCreated")
	}
	token1, ok := event.Args["token1"].(common.Address)
	if !ok {
		return fmt.Errorf("invalid token1 address in PoolCreated")
	}
	pool, ok := event.Args["pool"].(common.Address)
	if !ok {
		return fmt.Errorf("invalid pool address in PoolCreated")
	}
	...
	fee := "0"
	if v, ok := event.Args["fee"].(*big.Int); ok && v != nil {
		fee = v.String()
	}
	...
	if err != nil {
		return fmt.Errorf("failed to insert v3 pool: %w", err)
	}
...
func handleSwap(ctx context.Context, module *UniswapV3Module, event *core.ParsedEvent) error {
	...
	if amount0 == nil || amount1 == nil || sqrtPriceX96 == nil || liquidity == nil || tick == nil {
		...
		if len(data) >= 160 { // 5 * 32 bytes
			...
			if amount0 == nil {
				amount0 = parseSigned(word(0))
			}
			if amount1 == nil {
				amount1 = parseSigned(word(1))
			}
			if sqrtPriceX96 == nil {
				sqrtPriceX96 = new(big.Int).SetBytes(word(2))
			}
			if liquidity == nil {
				liquidity = new(big.Int).SetBytes(word(3))
			}
			if tick == nil {
				tick = parseSigned(word(4))
			}
		}
	}
	...
	err := module.db.Pool().QueryRow(ctx, dupCheckQuery,
		...
		func() string {
			if sqrtPriceX96 != nil {
				return sqrtPriceX96.String()
			}
			return "0"
		}(),
		func() string {
			if liquidity != nil {
				return liquidity.String()
			}
			return "0"
		}(),
		func() string {
			...
			if ok {
				...
				if module.isStablecoin(l0) && amount0 != nil {
					if module.priceRouter != nil {
						if p, ok := module.priceRouter.PriceTokenUSD(ctx, l0, minute); ok {
							...
							usd0 = mulStr(divBigIntByPow10Str(absBig(amount0), dec), p)
						}
					}
				...
				if module.isStablecoin(l1) && amount1 != nil {
					if module.priceRouter != nil {
						if p, ok := module.priceRouter.PriceTokenUSD(ctx, l1, minute); ok {
							...
							usd1 = mulStr(divBigIntByPow10Str(absBig(amount1), dec), p)
						}
					}
				...
				if usd != "" {
					...
					if module.priceRouter != nil {
						...
						if l0 == zilAddr {
							if p, ok := module.priceProvider.PriceZILUSD(ctx, minute); ok {
								_, _ = module.db.Pool().Exec(ctx, `
	UPDATE tokens
	SET price_usd = $2,
	    price_eth = 1,
	...
	if module.publisher != nil {
		payload := map[string]interface{}{
			...
			"sqrt_price_x96": func() string {
				if sqrtPriceX96 != nil {
					return sqrtPriceX96.String()
				}
				return "0"
			}(),
			"liquidity": func() string {
				if liquidity != nil {
					return liquidity.String()
				}
				return "0"
...
func handleMint(ctx context.Context, module *UniswapV3Module, event *core.ParsedEvent) error {
	...
	if len(event.Log.Topics) >= 2 {
		owner = common.BytesToAddress(event.Log.Topics[1].Bytes())
	}
	...
	err := module.db.Pool().QueryRow(ctx, dupCheckQuery,
		...
		func() string {
			if tickLower != nil {
				return tickLower.String()
			}
			return "0"
		}(),
		func() string {
			if tickUpper != nil {
				return tickUpper.String()
			}
			return "0"
		}(),
		func() string {
	...
	if module.priceProvider != nil {
		...
		if err := module.db.Pool().QueryRow(ctx, `SELECT token0, token1 FROM uniswap_v3_pools WHERE address = $1`, strings.ToLower(event.Address.Hex())).Scan(&t0, &t1); err == nil {
			...
			if ok {
				total := "0"
				if amount0 != nil && amount0.Sign() != 0 {
					if usd, ok := module.valueTokenAmountUSD(ctx, l0, minute, absBig(amount0)); ok {
						total = addStr(total, usd)
					}
				}
				if amount1 != nil && amount1.Sign() != 0 {
					if usd, ok := module.valueTokenAmountUSD(ctx, l1, minute, absBig(amount1)); ok {
						total = addStr(total, usd)
					}
				}
				if total != "0" {
...
func (m *UniswapV3Module) ensureToken(ctx context.Context, tokenAddress common.Address, firstSeenBlock uint64, firstSeenTimestamp int64) error {
	...
	if err := m.db.Pool().QueryRow(ctx, q, addr).Scan(&exists); err != nil {
		return err
	}
	if exists {
		return nil
